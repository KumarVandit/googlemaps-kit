...
export function extractWizGlobalString(html: string, key: string): string | undefined {
  const m = html.match(new RegExp(`"${key}"\\s*:\\s*"([^"]*)"`));
  return m?.[1];
}

/** Build label (`bl` query param) — not always present as cfb2h in Maps HTML. */
export function extractBuildLabel(html: string): string | undefined {
  return (
...
export function extractSessionId(html: string): string | undefined {
  return (
