...
import { TtlCache } from '../../../src/utils/async.js';
import { throwIfAborted, sleepAbortable as sleep } from '../../../src/utils/async.js';
import { runWithRequestContext, getRequestSignal } from '../../../src/utils/async.js';
import { pooledMap } from '../../../src/utils/async.js';
...
describe('hooks + tools surface', () => {
  ...
  it('grid() derives square bounds from near + spanKm without network', async () => {
    const maps = sdk({ warmOnCreate: false });
    // Validation fires before any request when neither bounds nor near given.
    await expect(maps.grid({ query: 'cafes' } as never)).rejects.toThrow(/bounds or near/);
  });
});
