...
describe('Building & Map Layer Services - Integration', () => {
  ...
  // 3D calls fetch real octree nodes, so they run against a single block
  // rather than the whole city.
  const cityBlockBounds = {
    ne: { lat: 37.7790, lng: -122.4160 },
    sw: { lat: 37.7740, lng: -122.4220 },
  };
  ...
  describe('Map3dService integration', () => {
    ...
    it('should have getMesh method available', () => {
      expect(typeof client.map.map3d.getMesh).toBe('function');
    });

    it('returns segmented buildings from the Earth octree', async () => {
      const buildings = await client.map.map3d.getBuildings({
        bounds: cityBlockBounds,
        minHeight: 10,
        maxTiles: 2,
      });
      expect(Array.isArray(buildings)).toBe(true);
      for (const building of buildings) {
        expect(building.height).toBeGreaterThanOrEqual(10);
        expect(building.outline.length).toBeGreaterThanOrEqual(3);
      }
    }, 60_000);

    it('returns a sampled terrain grid', async () => {
      ...
      expect(terrain.maxAlt).toBeGreaterThanOrEqual(terrain.minAlt);
    }, 60_000);
  });
    ...
    it('should support parallel service calls', async () => {
      try {
        const results = await Promise.all([
          ...
          client.map.map3d.getBuildings({ bounds: cityBlockBounds, maxTiles: 1 }),
          client.map.map3d.getTerrain({ bounds: cityBlockBounds, maxTiles: 1 }),
        ]);
